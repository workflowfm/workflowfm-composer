	...
	public void grappaPressed(Subgraph subg, Element elem, GrappaPoint pt,
		...
		if ((modifiers & (InputEvent.BUTTON2_MASK | InputEvent.BUTTON3_MASK)) != 0
			...
			if (subg.currentSelection != null) {
				...
				popup.addSeparator();
				if (subg.currentSelection instanceof Element) {
					popup.add(item = new javax.swing.JMenuItem(
					...
					if (outline == null) {
						if (pressedElem == elem && pt.distance(pressedPt) < 5) {
							...
							if (attr == null || attr.getValue().equals("\\N")) {
								attrs = new Attribute[] {
										new Attribute(Grappa.NODE, POS_ATTR, pt),
										new Attribute(
												Grappa.NODE,
												LABEL_ATTR,
												"Node"
								...
								attrs = new Attribute[] { new Attribute(
										Grappa.NODE, POS_ATTR, pt) };
							}
							Element el = subg.createElement(Grappa.NODE, null,
									attrs);
							if (el != null) {
								el.buildShape();
								subg.getGraph().repaint();
							...
							subg.getGraph().repaint();
								...
								&& elem.isNode()) {
							// create edge
							Object[] info = new Object[] { elem, null,
									pressedElem };
							Attribute[] attrs = new Attribute[] { new Attribute(
									Grappa.EDGE,
									POS_ATTR,
									new GrappaLine(
											new GrappaPoint[] {
													...
													((Node) elem)
															.getCenterPoint() },
											subg.getGraph().isDirected() ? GrappaLine.TAIL_ARROW_EDGE
													: GrappaLine.NONE_ARROW_EDGE)) };
							Element el = subg.createElement(Grappa.EDGE, info,
									attrs);
							if (el != null) {
								el.buildShape();
								subg.getGraph().repaint();
